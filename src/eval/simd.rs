...
mod tests {
    ...
    fn corpus(seed: u64) -> (ObjectStore, Vec<crate::object::ObjectId>) {
        use crate::object::descriptor::{ObjectDescriptor, Representation};
        use crate::object::{self, Constant, Cycle, Literal, Noise, ObjectData, Oscillator};
        use crate::universe::layout::Layout;
        let mut store = ObjectStore::new();
        let mut rng = crate::universe::prng::XoShiro256::from_seed(seed ^ 0x5EED);
        let mut ids: Vec<crate::object::ObjectId> = Vec::new();
        ...
        let samples: Vec<i32> = (0..ext).map(|_| next32(&mut rng) as i32).collect();
        let d =
            ObjectDescriptor::new(Representation::Literal, ext, Layout::Mono, Some(lr)).unwrap();
        let id = store
            .insert(
                d.clone(),
                ObjectData::Literal(Literal::new(&d, samples).unwrap()),
        ...
        let rd = ObjectDescriptor::new(Representation::PredictorResidual, ext, Layout::Mono, None)
            .unwrap();
        ...
        let rid = insert(
            ...
            ObjectData::Referenced(
                crate::object::reference::Referenced::checked(target, tr as i64, None).unwrap(),
            ),
        );
        ids.push(rid);
    ...
    fn random_voice(
        ...
        store: &ObjectStore,
        rng: &mut crate::universe::prng::XoShiro256,
        out_channels: u8,
    ) -> VoiceSpec {
        let next32 = |rng: &mut crate::universe::prng::XoShiro256| rng.next_u64() as u32;
        ...
        let instant = EnvelopeParams::new(0, 0, 1 << 16, 0).unwrap();
        let _ = instant;
        ...
        let (object_channel, route) = if layout == Layout::Stereo && next32(rng) % 3 == 0 {
        ...
        let loop_mode = if endless || cycle {
            ...
            match next32(rng) % 3 {
                ...
                _ => {
                    ...
                    LoopMode::Region(crate::object::LoopRegion::new(a, b).unwrap())
                }
            }
        };
